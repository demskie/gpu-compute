#ifndef BIG_UINT_RSHIFT_01
#define BIG_UINT_RSHIFT_01
...
void biguintRshift(in float a[BYTE_COUNT], inout float b[BYTE_COUNT], in float c[BYTE_COUNT]) {
    ...
    for (int i = 0; i < BYTE_COUNT; i++) {
        ...
        b[BYTE_COUNT-1] = float(byteCount <= 0.0) * b[BYTE_COUNT-1];
        byteCount -= 1.0;
    }
    count = mod(count, 8.0);
    ...
    for (int i = 0; i < 8; i++) {
        for (int j = BYTE_COUNT*8 - 1; j > 0; j--) bits[j] = bits[j-1];
        bits[0] = false;

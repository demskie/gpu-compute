...
void main() {
	...
	float offset = mod(floor(gl_FragCoord.x), 2.0);
	result.r += floatEquals(offset, 0.0) * sourceTexel.r;
	result.g += floatEquals(offset, 0.0) * sourceTexel.g;
	result.r += floatEquals(offset, 1.0) * sourceTexel.b;
	result.g += floatEquals(offset, 1.0) * sourceTexel.a;
	gl_FragColor = result;
}
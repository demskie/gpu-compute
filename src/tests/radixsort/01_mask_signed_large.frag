...
uniform sampler2D u_firstQuarter;
uniform sampler2D u_secondQuarter;
uniform sampler2D u_thirdQuarter;
uniform sampler2D u_fourthQuarter;
...
void main() {
	...
	vec4 mostSigFirstQuarterTexel  = texture2D(u_firstQuarter,  vec2(float(mostSigCoord.x) + 0.5, mod(float(mostSigCoord.y), u_textureWidth) + 0.5) / u_textureWidth);
	vec4 mostSigSecondQuarterTexel = texture2D(u_secondQuarter, vec2(float(mostSigCoord.x) + 0.5, mod(float(mostSigCoord.y), u_textureWidth) + 0.5) / u_textureWidth);
	vec4 mostSigThirdQuarterTexel  = texture2D(u_thirdQuarter,  vec2(float(mostSigCoord.x) + 0.5, mod(float(mostSigCoord.y), u_textureWidth) + 0.5) / u_textureWidth);
	vec4 mostSigFourthQuarterTexel = texture2D(u_fourthQuarter, vec2(float(mostSigCoord.x) + 0.5, mod(float(mostSigCoord.y), u_textureWidth) + 0.5) / u_textureWidth);
	vec4 mostSigTexel = floatGreaterThanOrEqual(float(mostSigCoord.y), 0.0 * u_textureWidth) * floatLessThan(float(mostSigCoord.y), u_textureWidth * 1.0) * mostSigFirstQuarterTexel
					  + floatGreaterThanOrEqual(float(mostSigCoord.y), 1.0 * u_textureWidth) * floatLessThan(float(mostSigCoord.y), u_textureWidth * 2.0) * mostSigSecondQuarterTexel
					  + floatGreaterThanOrEqual(float(mostSigCoord.y), 2.0 * u_textureWidth) * floatLessThan(float(mostSigCoord.y), u_textureWidth * 3.0) * mostSigThirdQuarterTexel
					  + floatGreaterThanOrEqual(float(mostSigCoord.y), 3.0 * u_textureWidth) * floatLessThan(float(mostSigCoord.y), u_textureWidth * 4.0) * mostSigFourthQuarterTexel;
	...
	vec4 firstQuarterTexel  = texture2D(u_firstQuarter,  vec2(float(targetCoord.x) + 0.5, mod(float(targetCoord.y), u_textureWidth) + 0.5) / u_textureWidth);
	vec4 secondQuarterTexel = texture2D(u_secondQuarter, vec2(float(targetCoord.x) + 0.5, mod(float(targetCoord.y), u_textureWidth) + 0.5) / u_textureWidth);
	vec4 thirdQuarterTexel  = texture2D(u_thirdQuarter,  vec2(float(targetCoord.x) + 0.5, mod(float(targetCoord.y), u_textureWidth) + 0.5) / u_textureWidth);
	vec4 fourthQuarterTexel = texture2D(u_fourthQuarter, vec2(float(targetCoord.x) + 0.5, mod(float(targetCoord.y), u_textureWidth) + 0.5) / u_textureWidth);
	vec4 texel = floatGreaterThanOrEqual(float(targetCoord.y), 0.0 * u_textureWidth) * floatLessThan(float(targetCoord.y), u_textureWidth * 1.0) * firstQuarterTexel
			   + floatGreaterThanOrEqual(float(targetCoord.y), 1.0 * u_textureWidth) * floatLessThan(float(targetCoord.y), u_textureWidth * 2.0) * secondQuarterTexel
			   + floatGreaterThanOrEqual(float(targetCoord.y), 2.0 * u_textureWidth) * floatLessThan(float(targetCoord.y), u_textureWidth * 3.0) * thirdQuarterTexel
			   + floatGreaterThanOrEqual(float(targetCoord.y), 3.0 * u_textureWidth) * floatLessThan(float(targetCoord.y), u_textureWidth * 4.0) * fourthQuarterTexel;
	...
	texel.r = floatGreaterThan(u_byteCount, 0.0) * floatLessThanOrEqual(u_byteCount, 1.0) * 0.0
			...
			+ floatGreaterThan(u_byteCount, 2.0) * floatLessThanOrEqual(u_byteCount, 64.0) * texel.r;
